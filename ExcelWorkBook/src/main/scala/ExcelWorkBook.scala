/**
  * Created by seino on 2016/08/01.
  */
...
//    object FileType extends Enumeration {
  //    val XLS, XLSX = Value
   // }
    ...
    var workbk: Workbook = null
    // ワークブック
    var style: CellStyle = null
    // セルスタイル
    var dft: DataFormat = null
    // データフォーマット
    var filenm: String = ""
    // ファイル名
    ...
    def createCellStyle = style = workbk.createCellStyle

    //セルスタイル作成
    def getCellStyle = style // セルスタイル取得

    def createDataFormat = dft = workbk.createDataFormat

    // データフォーマット作成
    def getDataFormat = dft // データフォーマット取得

    def createSheet(sn: String) = sheet = workbk.createSheet(sn)

    // ワークシート作成
    def getSheet = sheet // ワークシート取得

    def getSheetName(sn: Int) = workbk.getSheetName(sn)
...
//      if (shn.isEmpty) removeSheet(shn.get)
 //     else println("指定シート名が異常です")
    ...
    // seino
    def getSheetData(name:String) :ArrayBuffer[String] = {
      selectSheet("UN_Manuscript")
    ...
    def addRow(originalRow :org.apache.poi.ss.usermodel.Row) :Unit = {
      val startRowNumber = sheet.getLastRowNum + 1
      sheet.createRow(startRowNumber)

      val newRow = sheet.getRow(startRowNumber)

      for {index <- 0 until originalRow.getLastCellNum} {
        ...
        if (originalCell != null) {
          ...
          // セルのスタイルのコピー
          val newCellStyle = workbk.createCellStyle
    ...
    def copySheet(originalSheet :org.apache.poi.ss.usermodel.Sheet, revision:String) :Unit = {
      if (getSheet == null)
        createSheet(originalSheet.getSheetName)

      for(rowNum <- originalSheet.getFirstRowNum to originalSheet.getLastRowNum) {
        val row = originalSheet.getRow(rowNum)
        // add revision
        row.createCell(0).setCellValue(revision)
        // loop :cell
        if (row != null) {
          addRow(row)
        }
  ...
  def lsExcel(dir: String) : ArrayBuffer[String] = {
    ...
    val updateExcel = """(.*\\UN_for_translate_.*.xls)""".r
    ls(dir).map { e => e.getPath }.foreach { str =>
      str match {
        case updateExcel(x) => result += x
        case _ =>
      }
    }
    ...
    excelList.foreach { file =>
      ...
      excel.getSheetNameMap.foreach{
        case(key,value) if (key == "UN_Manuscript") => {
          ...
          copyExcel.copySheet( excel.getSheet, rev )
        }
